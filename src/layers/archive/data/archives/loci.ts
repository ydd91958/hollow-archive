...
export const loci: Archive[] = [
  {
    ...
    body: [
      ...
      {
        kind: 'text',
        text: '该日十七时四十一分至十八时二十二分，本站全部记录中断，历时四十一分钟。中断期间：纸带记录笔停留在同一位置；值班日志缺一页；井温自记曲线为一条直线。',
      },
      {
        kind: 'text',
        text: '十八时二十二分记录恢复。恢复后的第一行日志由当班值班员写下，全文为：「已复位。请勿询问。」该行之后另有一行被涂销，涂销层下可辨认出两个数字。',
      },
      { kind: 'margin', hand: '铅笔 · 极轻', text: '四十一分钟。纸带上没有断口。笔一直是抬着的。' },
      /* 玩家第一次看到「收容」二字。正式文件从不用这个词。 */
      {
        kind: 'margin',
        hand: '铅笔 · 另一支',
        text: '上面写的是登记。我们底下没人这么叫。这叫收容。收容完了，那东西就不在外面了。',
      },
      {
        kind: 'text',
